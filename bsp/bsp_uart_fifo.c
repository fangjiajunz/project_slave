...
static void PrintRxData(uint8_t _ch)
{
#if 0
...
/**
 * @brief  从发送缓冲区中取出所有数据
 * @param  _pUart : 串口结构体指针
 * @param  _pBuf  : 用户提供的缓冲区
 * @param  _usBufSize : 用户缓冲区大小
 * @return 实际读取的数据长度
 */
uint16_t UartReadAllTx(UART_T *_pUart, uint8_t *_pBuf, uint16_t _usBufSize)
{
    uint16_t usCount = 0;

    /* 临界区保护，防止中断修改 usTxWrite/usTxRead/usTxCount */
    DISABLE_INT();

    while ((_pUart->usTxRead != _pUart->usTxWrite) && (usCount < _usBufSize))
    {
        _pBuf[usCount++] = _pUart->pTxBuf[_pUart->usTxRead];

        if (++_pUart->usTxRead >= _pUart->usTxBufSize)
        {
            _pUart->usTxRead = 0; /* 环形回绕 */
        }
 ...
 * @param  _pBuf      : 接收缓冲区
 * @param  _usBufSize : 缓冲区大小
 * @param  _usTimeOut : 总超时时间(ms)，0 表示无限等待
 * @return 实际接收到的数据长度
 */
uint16_t Com_ReadLine(COM_PORT_E _ucPort, char *_pBuf, uint16_t _usBufSize, uint32_t _usTimeOut)
{
    uint8_t ucData;
    uint16_t pos = 0;
    uint32_t startTick = HAL_GetTick();
    uint32_t charTick = startTick;

    while (1)
    {
        /* 总超时检测 */
        if (_usTimeOut > 0 && (HAL_GetTick() - startTick) >= _usTimeOut)
        {
            _pBuf[pos] = 0;
            break;
        }

        /* 字符间超时: 收到首字节后 500ms 无新数据则结束 */
        if (pos > 0 && (HAL_GetTick() - charTick) >= 500)
        {
            _pBuf[pos] = 0;
            break;
        }
...
/**     @param _ucPort : 端口号(COM1 - COM8)
 *      @param _pExpect : 期望包含的字符串，如 "OK"
 *      @param _usTimeOut : 总超时时间(ms)
 *      @return 1 表示收到期望应答, 0 表示超时未收到
 */

uint8_t comWaitResponse(COM_PORT_E _ucPort, const char *_pExpect, uint32_t _usTimeOut)
{
    uint8_t ucData;
    uint8_t buf[128];
    uint16_t pos = 0;
    ...
    while (_pExpect[expectLen] != '\0')
        expectLen++;
    startTick = HAL_GetTick();

    while ((HAL_GetTick() - startTick) < _usTimeOut)
    {
        if (comGetChar(_ucPort, &ucData))
        {
            if (pos < sizeof(buf) - 1)
            ...
            {
                uint16_t i;
                for (i = 0; i <= pos - expectLen; i++)
                {
                    uint16_t j;
                    uint8_t match = 1;
                    for (j = 0; j < expectLen; j++)
                    {
                        if (buf[i + j] != (uint8_t)_pExpect[j])
                        {
                            match = 0;
                            break;
                        }
