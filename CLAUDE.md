...
LED: PA15(TX), PB6(RX)
蜂鸣器: PB3 (GPIO输出，原TIM2 CH2 PWM已改为GPIO直接控制)
  - TIM2用于1ms定时中断驱动TinyFrame超时机制
  - 蜂鸣器改用GPIO开关控制，阈值触发时响铃1秒
继电器: 风扇/水泵/加热器/LED (通过app_relay模块控制)
  - LED初始状态：开启（其他继电器初始关闭）
```
